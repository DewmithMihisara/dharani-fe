...
export async function printDeliveryNote(orderId, token = null) {
  ...
  const sharedDuration = (o.items || []).find(i => i.duration_months)?.duration_months || null
  ...
  const singerItems = (o.singerItems || []).map(i => ({
    ...
    qty: i.qty || 1,
    period: sharedDuration ? `${sharedDuration} Months` : '—',
...
<div class="doc-title">D E L I V E R Y &nbsp; N O T E</div>
...
<table class="items">
  ...
  <tfoot>
    <tr>
      ...
      <td class="c" style="font-size:8px">TOTAL INSTALMENT AMOUNT (Rs.)</td>
