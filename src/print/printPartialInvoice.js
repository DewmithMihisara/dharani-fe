...
export async function printPartialInvoice(orderId, token = null) {
  ...
  const itemRows = paidItems.map(i => `
    <tr>
      ...
      <td class="r pl">${fmt(Number(i.item_value || 0) * (i.qty || 1))}</td>
...
.remark-box{border:1px solid #000;border-radius:4px;padding:2px 8px;min-height:22px;
...
<table class="items">
  <thead>
    <tr>
      ...
      <th style="width:90px">Total Price (LKR)</th>
  ...
  <tfoot>
    <tr>
      <td colspan="4" class="r" style="font-size:12px">Total :</td>
