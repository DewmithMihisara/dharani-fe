...
import { uploadBadge } from '../api/inventoryApi'

// Expected header row (row 2), in order. Compared after whitespace-collapse + lowercase.
const DISPLAY_HEADERS = [
  'Product Category', 'Item', 'Model', 'Size', 'Name',
  'Transfer Price', 'Admin Cost', 'Sales & Promotion', 'Transport',
  'Total Cost', 'Price', 'VAT', 'Price with VAT',
...
function pct(part, base) {
  return base > 0 ? Math.round((part / base) * 10000) / 100 : 0
}
...
export default function BadgeUploadDialog({ onClose, onUploaded }) {
  ...
  async function handleUpload() {
    ...
    ws.eachRow((row, rowNumber) => {
      ...
      const modelName = cellText(row, 3)
      if (!category || !itemName || !modelName) return // skip empty / incomplete rows
      const transferPrice = cellNum(row, 6)
      const adminCost = cellNum(row, 7)
      const salesAndPromotion = cellNum(row, 8)
      const transport = cellNum(row, 9)
      const priceBeforeVat = cellNum(row, 11)
      const vat = cellNum(row, 12)
      items.push({
        id: null,
        modelId: null,
        ...
        transferPrice,
        adminCost,
        adminCostPct: pct(adminCost, transferPrice),
        salesAndPromotion,
        salesAndPromotionPct: pct(salesAndPromotion, transferPrice),
        transport,
        transportPct: pct(transport, transferPrice),
        totalCost: cellNum(row, 10),
        taxPct: pct(vat, priceBeforeVat),
        taxValue: vat,
        price: cellNum(row, 13), // Price with VAT — the stored price
        month6: cellNum(row, 14) || null,
        month12: cellNum(row, 15) || null,
        month18: cellNum(row, 16) || null,
        month24: cellNum(row, 17) || null,
      })
    })
    ...
    try {
      const res = await uploadBadge({ projectId: ctx.projectId, items }, token)
      if (res.status === 200) {
        ...
        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-xl overflow-hidden">
          ...
          <div className="bg-[#14213d] px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-white font-semibold text-sm">Upload Badge by Excel</span>
            </div>
            ...
            {phase === 'form' ? (
              <>
                ...
                <div className="rounded-lg bg-[#f6f8fc] border border-[#e3e9f3] p-4 text-xs text-[#555] leading-relaxed">
                  ...
                  <ul className="list-disc pl-4 flex flex-col gap-1">
                    ...
                    <li>Category, Item and Model are matched by name (created automatically if new) — check spelling.</li>
              ...
              <>
                ...
                <div className="flex justify-end gap-2">
                  ...
                  {succeeded && (
                    <Button type="button" onClick={() => onUploaded({ id: result.badgeId, badgeNumber: result.badgeNumber, status: 'PENDING' })}>
                      View Badge
                    </Button>
                  )}
                </div>
