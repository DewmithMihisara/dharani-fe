...
import { approveBadge, getItemsByBadge } from '../api/retailBadgeApi'
...
export default function RetailBadgeView({ badge, onClose, onApprove }) {
  ...
  useEffect(() => {
    ...
    load()
  }, [badge.id, offset, limit, searchTerm])
          ...
          <div className="bg-[#14213d] rounded-t-2xl px-6 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <div className="relative">
                <Search size={13} className="absolute left-2.5 top-1/2 -translate-y-1/2 text-[#6b7a99]" />
                <input
                  value={search}
                  onChange={e => setSearch(e.target.value)}
                  placeholder="Search category, item, model…"
                  className="pl-7 pr-6 py-1 w-56 rounded-md border border-white/20 bg-white/5 text-xs text-white placeholder-[#6b7a99] focus:outline-none focus:border-white/40"
                />
                {search && (
                  <button
                    onClick={() => setSearch('')}
                    className="absolute right-1.5 top-1/2 -translate-y-1/2 text-[#6b7a99] hover:text-white cursor-pointer"
                  >
                    <X size={12} />
                  </button>
                )}
              ...
              <div className="overflow-auto max-h-[60vh]">
                <table className="min-w-max w-full text-xs border-collapse">
                  ...
                  <tbody>
                    {loading ? (
                      <tr>
                        <td colSpan={12} className="px-3 py-6 text-center text-sm text-[#bbb]">Loading…</td>
                      </tr>
                    ) : items.map((e, i) => (
                      <tr
                        key={e.id}
